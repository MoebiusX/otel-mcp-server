...
- **MCP 2026-07-28 support.** The new revision makes the protocol stateless — the `initialize` handshake and `Mcp-Session-Id` are gone (SEP-2575, SEP-2567) — so a 2026 client's first request can be a bare `tools/call` and no session id is issued or required. Pre-2026 clients keep working on the same endpoint: the revision is resolved per request from `MCP-Protocol-Version`, or inferred from the 2026 routing headers / `_meta` client info when the header is absent. Also implemented: `Mcp-Method`/`Mcp-Name` routing-header validation with rejection on header/body disagreement (SEP-2243); `server/discover` returning capabilities, supported revisions, and the skill inventory; `ttlMs`/`cacheScope` cache hints on list and resource-read results (SEP-2549, `cacheScope: session` because tool lists are scope-filtered per credential — tool calls are never marked cacheable); and a reverse-DNS `extensions` capability map declaring `io.modelcontextprotocol/enterprise-managed-authorization` when configured (SEP-2133). New `src/mcp-spec.ts` models spec revisions and feature flags so nothing branches on version strings. SDK v1 tops out at protocol `2025-11-25` and rejects newer version headers, so this lives in our HTTP layer above the SDK; `/health` reports supported revisions.
- **W3C Trace Context propagation (SEP-414).** `traceparent`/`tracestate`/`baggage` arriving in a tool call's `_meta` — or as inbound HTTP headers from a proxy — now propagate onto every outbound backend query, so a single trace spans host → MCP server → Prometheus/Jaeger/Loki instead of producing unrelated spans. Carried through `AsyncLocalStorage` (`src/request-context.ts`) so all ~120 existing tool call sites and their pre-baked fetchers get it without modification. `_meta` wins over HTTP headers, malformed or CR/LF-bearing values are dropped rather than forwarded, and nothing is invented when the client propagated nothing. New `mcp_spec_requests_total{version,mode}`, `mcp_trace_context_propagated_total{source}`, and `mcp_routing_header_rejections_total{header}` metrics.
...
- **Lineage revocation could be used across tenants once a token had rotated.** The `root_id` ownership check resolved the owner with a token-id lookup, but the generation-0 record whose id *is* the root id is pruned on the second rotation and swept after its grace window — so the guard silently evaluated to "allowed" while revocation still worked off the lineage index. Any scope-restricted key could then kill any other key's live session, which is the steady state for every refresh-using client. Ownership is now resolved from the lineage's live members (new `getLineage` store primitive) and fails closed.
- **A malformed entry in the client-keys file no longer breaks authentication.** `validateClientKey` hashes every entry without early exit; an entry missing a string `key` (operator JSON is not schema-validated) raised and took down the request handler that was merely checking a credential against the *other*, valid keys. Malformed entries are skipped.
- **A stateless GET no longer hangs forever and leaks an `McpServer`.** On the 2026 path every method reached a fresh transport, and a GET opened the SDK's standalone SSE stream whose body never ends — the request never completed, its cleanup never ran, and each one pinned a server for the process lifetime. In a session-less protocol there is no standalone stream and no session to delete, so only POST is accepted; GET/DELETE get `405`.
- **Routing-header validation covers the whole batch.** Only the first message was checked, so a batch could show a gateway a benign `Mcp-Method` while a later message called something the gateway would have blocked — the smuggling SEP-2243 exists to prevent.
- **Cache hints are correlated by request id.** One hint derived from a batch's first message was stamped on every response, advertising a live `tools/call` result as cacheable for five minutes; an agent could then reuse a stale telemetry answer.
- **`Accept` is only rewritten for a 2026 POST.** The rewrite was unconditional, so a pre-2026 client asking for JSON only received an SSE response it cannot parse instead of an honest `406`, and a GET was handed a stream it never asked for.
- **Trace-context values that `fetch()` cannot encode are dropped, not forwarded.** `tracestate`/`baggage` were only checked for CR/LF, but `fetch()` throws on any header value containing a character above U+00FF or a DEL — so an emoji in `baggage` turned every backend query in that request into a hard failure. Values are now restricted to the printable-ASCII set W3C defines these fields over.
- **An unparseable protocol version no longer implies stateless serving.** Any unrecognized value was treated as a future revision, but these are string comparisons and letters sort after digits, so `garbage` counted as newer than `2026-07-28`. Only well-formed dates later than the newest known revision are treated as future ones. `2024-10-07` was also missing from the known-revisions list — a version the bundled SDK accepts — and a guard test now asserts the list stays a superset of the SDK's.
- **Session-gauge drift and McpServer leak.** A request that created a transport but was never assigned a session id (e.g. a non-initialize request carrying an unknown session id) was never closed, so `mcp_active_sessions` drifted upward permanently and the connected `McpServer` was only reclaimed by GC. The pair is now closed when no session id was assigned.
- **`serializeMetrics` no longer needs a hand-maintained list.** It derives the exposition from the registry, so a metric added to the registry but forgotten in the serializer can no longer be silently absent from `/metrics`.
